...
import { CREWS, CURRENT_USER, scoreBg, scoreColor, availDot, availLabel, getInitials } from '@/lib/data'
...
export default function ProfilePage({ params }: { params: { slug: string } }) {
  ...
  const isOwn = crew.id === CURRENT_USER.id
        ...
        <div className="grid md:grid-cols-3 gap-6">
          ...
          <div className="md:col-span-2 space-y-4">
                ...
                <div className="flex-1">
                  ...
                  {crew.availability_note && (
                    <p className="text-xs mt-2 italic" style={{ color: 'var(--text-2)' }}>
                      <span className={`inline-block w-1.5 h-1.5 rounded-full mr-1.5 ${availDot(crew.availability)}`} />
                      {crew.availability_note}
                    </p>
                  )}
                </div>
            ...
            <div className="card p-5">
              ...
              <div className="flex flex-wrap gap-2">
                {crew.equipment.map(e => (
                  <span key={e} className="text-xs px-2.5 py-1 rounded-lg"
                    style={{ background: 'var(--bg-3)', border: '1px solid var(--border)', color: 'var(--text-2)' }}>{e}</span>
                ))}
              </div>
            </div>
            ...
            <div className="card p-5">
              <div className="flex items-center justify-between mb-4">
                ...
                {/* FIX 1: Leave a review button on reviews section */}
                {!isOwn && (
          ...
          <div className="space-y-4">
            ...
            <div className="card p-5 text-center">
              ...
              <div className={`inline-flex flex-col items-center px-6 py-4 rounded-2xl ${scoreBg(crew.clearscore)}`}>
              ...
              <div className="mt-3 pt-3" style={{ borderTop: '1px solid var(--border)' }}>
                <p className={`text-2xl font-bold ${scoreColor(hireAgain)}`}>{hireAgain}%</p>
            ...
            <div className="card p-4 space-y-2">
              ...
              {/* FIX 1: Leave a review button in sidebar too */}
              {!isOwn && !reviewDone && (
                <button onClick={() => setShowReview(true)}
            ...
            <div className="card p-4">
              ...
              <div className="flex items-center gap-2 mb-1">
                <span className={`w-2 h-2 rounded-full ${availDot(crew.availability)}`} />
                <span className="text-sm font-medium" style={{ color: 'var(--text-1)' }}>{availLabel(crew.availability)}</span>
              </div>
              {crew.availability_note && (
...
function RequestModal({ crew, onClose, onSend }: { crew: any, onClose: () => void, onSend: () => void }) {
...
function ReviewModal({ crew, onClose, onSend }: { crew: any, onClose: () => void, onSend: () => void }) {
